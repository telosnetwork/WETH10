...
contract TestFlashLender is IERC3156FlashBorrower {
    ...
    function onFlashLoan(address sender, address token, uint256 value, uint256, bytes calldata data) external override returns(bytes32) {
        ...
        if (action == Action.NORMAL) {
            flashBalance = IWTLOS10(lender).balanceOf(address(this));
        } else if (action == Action.WITHDRAW) {
            IWTLOS10(lender).withdraw(value);
            flashBalance = address(this).balance;
            IWTLOS10(lender).deposit{ value: value }();
        } else if (action == Action.STEAL) {
            // Do nothing
        } else if (action == Action.REENTER) {
            bytes memory newData = abi.encode(Action.NORMAL);
            IWTLOS10(lender).approve(lender, IWTLOS10(lender).allowance(address(this), lender) + value * 2);
            IWTLOS10(lender).flashLoan(this, address(lender), value * 2, newData);
        }
    ...
    function flashLoan(address lender, uint256 value) public {
        ...
        IWTLOS10(lender).approve(lender, value);
        IWTLOS10(lender).flashLoan(this, address(lender), value, data);
    }
